...
using mux_demux = sel::eng6::proc::mux_demux <ut_traits::input_size, ut_traits::output_frame_size>;


struct sig_gen_ramp : sel::eng6::Processor01A<ut_traits::input_size>
{
	size_t c = 0;
	void process() final
	{
...
void run() {
	...
	struct null_sink : sel::eng6::Processor1A0<ut_traits::output_frame_size>
	{
		void process() final {}
	} logger;
	...
	sel::eng6::semaphore sem;

	sel::eng6::schedule input_schedule(&sem, input_proc);
	sel::eng6::schedule output_schedule(&mux_demux, output_proc);

//			input_schedule.init();
//			output_schedule.init();

	// try invalid triggers
	sel::eng6::schedule illegal_input_schedule(&mux_demux, input_proc);
	sel::eng6::schedule illegal_output_schedule(&sem, output_proc);
