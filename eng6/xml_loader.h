		...
		class xml_loader : public singleton<xml_loader>
		{
			...
			bool loadProcessorDefinitions(xml_document& doc)
			{
				...
				for (auto xpathnode : xpath_procdefs) {
					...
					eng6::proc::compound_processor& compound_proc = eng6::proc::compound_processor::create_ref(create_params);
					...
					for (auto xpathnode : xpath_procs) {
						...
						auto *proc = dynamic_cast<eng6::ConnectableProcessor *>(abstract_factory::create_from_type(create_params.at("type"), create_params));
					...
					for (auto xpathnode : xpath_consts) {
						...
						auto c = sel::eng6::Const::create_ref(create_params);
					...
					for (auto xpathnode : xpath_connections) {
						...
						auto *from_proc = dynamic_cast<eng6::ConnectableProcessor *>(obj);
						auto *from_const = dynamic_cast<eng6::Const *>(obj);
						auto *to = dynamic_cast<eng6::ConnectableProcessor *>(&instance_manager::from_handle(to_id));
						if (!to)
			...
			bool loadRepeaters(xml_document& doc)
			{
				...
				for (auto xpathnode : xpath_repeaters) {
					...
					eng6::periodic_event& repeater = eng6::periodic_event::create_ref(create_params);
			...
			bool loadSchedules(xml_document& doc)
			{
				...
				for (auto xpathnode : xpath_schedules) {
					...
					auto trigger_ = instance_manager::ptr_from_handle<eng6::semaphore>(trigger_id);
					if (!trigger_)
					...
					eng6::scheduler::get().add(trigger_, *action_);
